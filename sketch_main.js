// --- Main p5.js Sketch File ---

function preload() {
    logOutput("Preloading assets...");
    // Load Niki images
    // Encoded images included directly for simplicity
    nikiUp = loadImage("data:image/png;base64,");
    nikiLeft = loadImage("data:image/png;base64,");
    nikiDown = loadImage("data:image/png;base64,");
    nikiRight = loadImage("data:image/png;base64,");

    // Fetch and compile the grammar file
    loadAndCompileGrammar();
    logOutput("Preload finished.");
}

function setup() {
    logOutput("Setting up canvas and listeners...");
    let canvas = createCanvas(ARR_COLS * RECT_WIDTH, ARR_ROWS * RECT_HEIGHT);
    canvas.parent('canvas-container'); // Place canvas in the div

    // Disable right-click context menu SPECIFICALLY on the canvas
    canvas.elt.addEventListener('contextmenu', (e) => {
        e.preventDefault();
    });

    // Initialize Niki and Grid state
    resetNiki();

    // Setup Button Listeners
    document.getElementById('run-button').addEventListener('click', runCode);
    document.getElementById('reset-button').addEventListener('click', resetNiki);
    document.getElementById('builder-mode-checkbox').addEventListener('change', handleBuilderModeToggle);

    // Setup Textarea Tab Handling
    const editor = document.getElementById('code-editor');
    handleTabInTextarea(editor);

    // Setup mouse interaction handlers (these will check builderMode internally)
    // p5 registers these globally, but they are routed to builder logic
    canvas.mousePressed(builderMousePressed); // Use specific p5 handler attached to canvas
    canvas.mouseClicked(builderMouseClicked); // Use specific p5 handler attached to canvas


    logOutput("Setup complete. Ready.");
    updateBallCounterDisplay(); // Show initial ball count
}

function draw() {
    background(220); // Clear background each frame

    // Drawing Order: Base -> Niki -> Items on Top
    drawGridBase();   // Draw walls and empty squares
    drawNiki();       // Draw Niki's image and ball count on it
    drawGridItems();  // Draw balls in squares (will appear over Niki if on same square)
}

function resetNiki() {
    logOutput("Resetting Niki state and grid...");
    // Reset Niki's position, direction, balls
    nikiCol = Math.floor(ARR_COLS / 2);
    nikiRow = Math.floor(ARR_ROWS / 2);
    nikiDirection = (1 << 30); // North
    nikiNumOfBalls = 5; // Default starting balls

    // Recreate the grid
    createGrid(); // This function now includes logging

    // Clear user-defined procedures and errors
    procedures = {};
    clearError();

    // Update displays
    updateBallCounterDisplay();
    logOutput("Reset complete.");
} 